...
const TimeControls: React.FC<TimeControlsProps> = observer(
  ({ onSortByChange, onJumpToStep, onSetReference, onToggleThumbnailStep }) => {
    ...
    const [enlarged, setEnlarged] = useState<{ step: number; type: 'low' | 'medium' | 'high'; url: string } | null>(null);

    const canRemove = volumeStore.thumbnailSteps.length > 1;

    const renderThumb = (step: number, type: 'low' | 'medium' | 'high') => {
      const img = volumeStore.getThumbnailImage(step, type);
      const isRef = step === referenceStep;
      const rangeLabel = type === 'low' ? '低密度' : type === 'medium' ? '中密度' : '高密度';
      return (
        <div
          key={`${type}-${step}`}
      ...
      <>
        <div className="time-controls">
          <div className="thumb-scroll-wrapper">
            ...
            <div className="thumb-rows">
              <div className="thumb-row-label thumb-row-label--medium">
                <span>中密度</span>
              </div>
              <div className="thumb-cards">{volumeStore.thumbnailSteps.map(s => renderThumb(s, 'medium'))}</div>
            </div>
            <div className="thumb-rows">
