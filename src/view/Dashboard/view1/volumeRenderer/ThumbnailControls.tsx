...
const ThumbnailControls: React.FC = observer(() => {
  return (
    <div className="thumb-ctls standalone">
      ...
      <div className="thumb-ctl-group">
        <label>中密度</label>
        <Slider min={0} max={1} step={0.01} value={volumeStore.thumbnailMediumRange[0]} onChange={(e) => volumeStore.setThumbnailMediumRange(e.value as number, volumeStore.thumbnailMediumRange[1])} className="h-3 flex-1 mx-0.5" />
        <Slider min={0} max={1} step={0.01} value={volumeStore.thumbnailMediumRange[1]} onChange={(e) => volumeStore.setThumbnailMediumRange(volumeStore.thumbnailMediumRange[0], e.value as number)} className="h-3 flex-1 mx-0.5" />
        <span className="ctl-range-val">
          {volumeStore.thumbnailMediumRange[0].toFixed(2)}-{volumeStore.thumbnailMediumRange[1].toFixed(2)}
        </span>
      </div>
